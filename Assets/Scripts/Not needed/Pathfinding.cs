...
public class Pathfinding : MonoBehaviour {
	...
	{
		timer = 0;
		currentNodePosition = GridReference.FinalPath[currentNodeIndex].vPosition;
		startMovePosition = StartPosition.position;
	}
    ...
    private void Update()//Every frame
    {
		...
		float distance = Vector3.Distance(TargetPosition.position, oldTargetPosition); 
		if(distance > 0) {
			...
			if(StartPosition.position != currentNodePosition) {
				StartPosition.position = Vector3.Lerp(startMovePosition, currentNodePosition, timer);
			}  else {
				if(currentNodeIndex < GridReference.FinalPath.Count - 1) {
					currentNodeIndex++;
					checkNode();
				}
			}
